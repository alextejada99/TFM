...
output_csv   = "resultados_validacion_noOOD.csv"

# === Lista ordenada de las 38 clases entrenadas ===
class_names = [
...
for fname in sorted(os.listdir(test_dir)):
    ...
    # Extrae etiqueta verdadera antes del último "_"
    ...
    x_np = preproc(img).unsqueeze(0).numpy()

    # Inferencia ONNX
    logits = sess.run(None, {input_name: x_np})[0]   # (1,38)
    probs  = np.exp(logits) / np.sum(np.exp(logits), axis=1, keepdims=True)
    idx    = int(np.argmax(probs, axis=1)[0])
    conf   = float(probs[0, idx])
    pred   = class_names[idx]   # siempre una de las 38
...
# Métricas (solo clases con soporte >0)
...
# Muestra aleatoria de 4 imágenes
